...
>
> **⏳ PENDIENTE post-migración (NO urgente, funcionan vía redirect mientras tanto):**
> 1. **Google Ads — URLs:** cambiar URL final de campaña + las ~10 sitelinks + `/whatsapp` a `alttahomescancun.mx` (solo intercambiar dominio, ruta igual; NO borrar/recrear sitelinks para no perder CTR). Verificado que hoy NO dan error: las URLs viejas terminan en HTTP 200 vía 301. **NO pausar la campaña** (resetea aprendizaje + frena crédito).
> 2. **GBP (3 perfiles)** — apuntar sitio web al dominio nuevo.
> 3. **Meta** — verificar dominio nuevo en Business Settings (iOS/AEM).
...
>
> **🚫 DECIDIDO NO HACER ahora — renombrar/acortar rutas de silos:** Florencio volvió a plantear acortar rutas (`/jardines-del-sur-6`, `/jardines-del-sur-6/capua`) quitando `/desarrollos-cancun/`. RECHAZADO por timing: hacerlo en la misma ventana que la migración de dominio tira la re-indexación recién arrancada, duplica el bache de QS, exige 301 por ruta + re-actualizar Ads/GBP/Meta/sitemap. **Regla: UNA migración a la vez.** Si se hace, será proyecto APARTE tras ~4-8 sem (cuando el dominio asiente), y con guiones + sin abreviar (la versión "jardinesdelsur6/depacapua" es retroceso de SEO).
