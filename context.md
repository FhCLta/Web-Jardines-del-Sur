...
>
> **Lo aplicado en `components/MortgageCalculator.jsx`:**
> - **`maxCash = Math.ceil(totalCost)`** (precio + gastos de escrituración) = pago de contado. Se eliminó el `× 0.3`.
> - **⭐ El slider cae EXACTO en el total, sin "faltantes".** Como avanza de `CASH_STEP` (5,000) en 5,000 desde el mínimo, el último escalón casi nunca coincide con el total ($1,945,000 de $1,948,380). Solución: la barra se estira al siguiente múltiplo (**`sliderMax`**, siempre alineado al step → siempre alcanzable) y el **valor** se limita a `maxCash` en el `onChange`. Resultado: el final de la barra es el total al peso. (Efecto cosmético conocido y despreciable: al quedar `cashTotal` desalineado del step, el navegador puede pintar el thumb 1 escalón antes del final — <0.3% de la barra, invisible.)
> - **⚠️ ESTADO "CONTADO" NUEVO (`calc.isCash`) — era obligatorio, no adorno:** al llegar a contado el `principal` es 0 y **cuatro rótulos mentían**. Todos corregidos, en la línea de la regla de la sesión 44 (*un cálculo mal etiquetado es PEOR que ningún cálculo*):
>   1. **Seguros.** `insurance` seguía cobrando el seguro de daños sobre el avalúo → pintaba **~$357/mes a alguien que ya pagó la casa completa**. Vida, daños y comisión de administración los exige el BANCO como condición del préstamo: sin préstamo no existen → `total = 0`.
>   2. **Badge.** "Estrenas SIN ENGANCHE — el crédito cubre precio y gastos" se contradice solo en contado. Ahora: *"Pago de **contado** — sin crédito y sin mensualidad"*.
>   3. **Número grande.** "$0 /mes" no informa nada → en contado muestra **el desembolso real** (precio + gastos) con la etiqueta *"Pago único de contado*"* / *"en total"*.
>   4. **`creditSub`** decía "Cubre el resto tras tu aportación · **0% del avalúo**" → ahora *"No necesitas crédito: tu aportación cubre precio y gastos"*. Y **"Ingreso sugerido" se OCULTA** (saldría "$0" y se lee como "no necesitas ingresos").
> - **Mensaje de WhatsApp:** en contado no aplica el "X al mes a N años" (no hay mensualidad ni plazo) → manda el monto total. Se extrajo a `waResultado`/`waInfonavit` para no triplicar la plantilla.
> - **Bug de borde encontrado de paso y corregido:** el link de WhatsApp de la caja del modo manual está visible ANTES de escribir nada, y salía con *"precio $0 … $0 al mes"* (pre-existente). Ahora, con los campos vacíos manda un mensaje limpio ("no tengo a la mano el valor de avalúo, ¿me ayudas?"). El guard `totalCost > 0` en `isCash` evita además anunciar un "pago de contado de $0" en ese mismo estado.
>
> **Verificado:** build limpio; recalculados los 11 modelos del inventario (requiredCash y totalCost cuadran con el sitio — Capua $1,948,380, mínimo de Flamboyán $89,735 ✓); HTML compilado muestra `min="0" max="1950000" step="5000"`.
>
> **⏳ PENDIENTES/DECISIONES para Florencio (NO se tocaron, requieren su criterio):**
> - **(a) Créditos irrisorios.** Un escalón antes del contado el crédito queda en ~$3,000. La aritmética es correcta, pero **ningún banco presta esos montos** (el mínimo real ronda los $100-200k). ¿Ponemos un piso ("por debajo de $X ya conviene contado") o se deja? **Falta el dato del monto mínimo real de la banca.**
> - **(b) Badge con aportación voluntaria alta.** Si en Capua subes el efectivo a $525,000, el badge SIGUE diciendo "Estrenas sin enganche — el crédito cubre precio y gastos" mientras el crédito cubre solo el 63.83% (se ve en la captura que mandó Florencio). Es **pre-existente** y defendible (significa "este modelo no EXIGE enganche"), pero roza el patrón de rótulo que no cuadra. Se dejó como estaba a propósito: es decisión de copy.
> - **(c) Plazo y tasa** siguen activos en contado aunque ya no afecten el resultado. Se dejaron habilitados para que el panel no parpadee al mover el efectivo; si molesta, se pueden atenuar.

> **📌 "Nombre del asesor Florencio en el sitio" para consistencia NAP + defensa de perfiles Google Business (sesión 48, 25 jul 2026). PASO 2 (FOOTER) ✅ HECHO Y DESPLEGADO; el resto sigue apuntado.**
>
> **✅ HECHO (sesión 48) — FOOTER con asesor + redes, en las 10 páginas.** Componente reutilizable nuevo **`components/FooterSocial.jsx`** (+`.module.css`): línea discreta *"Florencio Hurtado · Asesor inmobiliario"* + iconos redondos a **Facebook** (`profile.php?id=61564048801892`) e **Instagram** (`instagram.com/florenciorealestate`). Insertado (`<FooterSocial />` + import `@/components/FooterSocial`) dentro del `.footerBottom` de los 10 footers duplicados del sitio: `app/page.tsx`, `calculadora-hipotecaria`, `preguntas-frecuentes`, `blog`, `blog/[slug]`, `_lib/SiloPage`, `_lib/CatalogPage`, `_lib/ModelPage`, `_lib/PromosPage`, `jardines-del-sur-7`. Estilo en tonos blancos translúcidos (los footers son navy #080f1c). Build limpio + deploy OK. **⚠️ Deuda técnica: el footer está copiado en 10 archivos** — si algún día se toca el pie conviene extraer un `<SiteFooter />` único; por ahora `FooterSocial` centraliza al menos el dato del asesor. **Las URLs de FB e IG servirán también para el `sameAs` del JSON-LD (paso 3).**
>
> **CONTEXTO (perfiles Google Business de Florencio):** tiene 4 perfiles GBP — Jardines del Sur 6, JdS7, La Rioja 2, Lirios Residencial 2 — nombrados como los desarrollos + "| Florencio Hurtado - Altta Homes" (JdS6 dice "Altta Homes **Cancun**", los otros 3 sin ciudad). La **competencia lo está atacando/reportando** los perfiles; Florencio dijo que **de eso se encarga él** (no es tarea de código). Diagnóstico que se le dio:
> - 🔴 **JdS6 sale "Duplicado"** (choque con el perfil oficial del desarrollador) = la única bandera roja real y el vector de ataque. La resuelve él.
> - El **pin/dirección = ubicación del desarrollo es CORRECTO** (es adonde manda al cliente) — Florencio lo confirmó como criterio. OJO: JdS6 muestra la dirección de su **oficina (Av. 127)**, no del desarrollo → verificar si su venta de JdS6 sí es en la oficina (quedó pregunta abierta con él).
> - JdS6: quitarle "Cancun" del nombre para igualar el formato de los otros 3.
>
> **LA IDEA (de Florencio, tras sugerencia de Google de "poner tu nombre en el sitio web"):** que el nombre "asesor Florencio" aparezca en la web de forma **consistente pero DISCRETA**, sin romper el look "oficial"/premium de la marca Altta Homes Cancún. Beneficio triple: (1) **consistencia NAP** que amarra los 4 perfiles GBP con el sitio → blinda contra los reportes de la competencia; (2) **E-E-A-T** (persona real detrás, clave en inmobiliario); (3) es honesto con [[user_florencio_role]] (asesor independiente).
>
> **PLAN CONCRETO A EJECUTAR (cuando Florencio dé el OK):**
> 1. **⭐ Botón flotante de WhatsApp con etiqueta ROTATIVA** (idea puntual de Florencio): que la mayoría de las veces diga "Informa por WhatsApp" y **de vez en cuando** "Asesor Florencio en línea" — proporción tipo **~5:1** (5 veces el genérico por 1 vez el nombre). Así el nombre sale sutil, no dominante. Definir si rota por carga de página, por intervalo o aleatorio ponderado.
> 2. **Footer:** mención consistente de "Florencio Hurtado — Asesor inmobiliario" (aparece en todas las páginas → refuerza NAP en cada URL). Discreto, sin robar protagonismo a la marca.
> 3. **⭐ Pieza técnica fuerte (recomendación de Claude, invisible al cliente):** nodo JSON-LD **`RealEstateAgent`/`Person`** "Florencio Hurtado" con **`sameAs` apuntando a los 4 perfiles de Google Business** — es lo que de verdad le dice a Google "esta persona = estos perfiles = este sitio". (Requiere las URLs de los 4 perfiles GBP; ya se tiene la de JdS7 del trabajo previo.)
> 4. Opcional (si Florencio lo pide): mini sección "Sobre tu asesor".
>
> **⚠️ NOTA de criterio:** para SEO/NAP la consistencia SUMA y lo aleatorio RESTA — el nombre debe ir **igualito y bien escrito** ("Florencio Hurtado") en los pocos lugares elegidos. "Sutil pero fijo", no "escondido al azar". La rotación del botón flotante es la excepción deliberada (es efecto visual, no dato NAP). **Pendiente de Florencio:** dar OK y decir qué tan visible lo quiere (mínimo = footer + JSON-LD; medio = + botón rotativo; máximo = + sección "Sobre tu asesor").

> **🚧 EN PROGRESO sesión 46 (25 jul 2026) — PÁGINAS-CATÁLOGO COMPARTIBLES + BROCHURE PDF. Piloto "Casas de JdS6" desplegado OCULTO (noindex); se replicará a TODOS los desarrollos.**
>
> **La idea (de Florencio):** que la web sea la fuente de info y tráfico. En vez de mandar PDFs muertos por WhatsApp, tener páginas-catálogo por grupo de producto que sirven DOBLE: (a) presentación compartible con botón de descarga PDF, y (b) silo SEO que posiciona búsquedas de intención ("casas en jardines del sur 6", "departamentos en jardines del sur 6", etc.). Se hará para: **Casas JdS6, Departamentos JdS6** (los 2 nuevos de verdad, JdS6 es el único con ambos tipos), y las demás (La Rioja casas / Lirios / JdS7) que casi ya son su silo — solo les faltaría el bloque amenidades + botón PDF con la misma plantilla.
>
> **Piloto hecho: `/jardines-del-sur-6/casas`** (Flamboyán, Ceiba, Tabachín, Noni). Arquitectura REUTILIZABLE:
> - **`app/(desarrollos)/_lib/CatalogPage.tsx`** — componente genérico `{ slug, kind: "casas"|"departamentos" }`. Filtra el inventario por tipo (`getModelType`), arma hero compacto + grid de `PropertyCard` + bloque `AmenitiesSection` (excepción deliberada: aquí SÍ van amenidades, [[hero-aspiracional-obra-abajo]] no aplica) + el brochure imprimible. Reusa `silo` styles.
> - **`buildCatalogMetadata(slug, kind)`** en `dev-meta.ts` — título largo SEO + descripción + canonical. Cada página nueva = una ruta de 8 líneas (ver `jardines-del-sur-6/casas/page.tsx`).
> - **`app/(desarrollos)/catalog.module.css`** — hero compacto + secciones + TODO el `@media print` del brochure.
>
> **DECISIONES de Florencio (aplican a todas las hermanas):**
> - **URL corta** `/dev/casas` (fácil de dictar por WhatsApp) + **título largo con marca** "Casas en Jardines del Sur 6 | Altta Homes Cancún" (la marca es "Altta Homes Cancún", ya trae Cancún). NO usar slug-marca tipo `alttahomes-casas-en...` en la URL.
> - **Enlazada pero NO en el navbar**: irá en sitemap + enlace discreto desde el silo (pendiente, paso de "enganches"). El veto del navbar sigue.
> - **Links a hermanas**: cada catálogo tendrá botones a los otros ("Ver departamentos", "Ver casas La Rioja") — se agregan conforme existan; el de deptos apunta provisional al silo de JdS6.
> - Título desalineado ("6" huérfano) → `text-wrap: balance` en h1/h2.
>
> **⭐ BROCHURE PDF — decisión clave: es PRE-GENERADO, no `window.print()`.** Se probó `window.print()` y falló en 2 cosas que Florencio cazó: (a) los **links no quedaban clicables** al imprimir desde celular; (b) dependía de que el usuario pusiera "márgenes: ninguno" + "gráficos de fondo". **Solución:** se genera el PDF con **puppeteer `page.pdf()`** (que SÍ embebe los `<a href>` absolutos como hipervínculos y respeta márgenes 0) y se sirve como **archivo estático** en `public/brochure-pdf/{slug}-{kind}.pdf`. El botón "Descargar folleto (PDF)" apunta a ese archivo. Resultado: **6 páginas, 1.1 MB, 36 links clicables**, portada full-bleed, se ve idéntico siempre.
> - **Estructura del brochure** (en el `@media print` de catalog.module.css + markup en CatalogPage): portada full-bleed (foto hero + overlay navy) → 1 página por casa (foto grande + precio + specs en chips + distribución nivel por nivel + CTA dorado clicable a la ficha del modelo) → página de amenidades (fotos + chips) + tarjeta navy de contacto (QR + link + WhatsApp + dirección). Header "AlttaHomes Cancún" en cada página. Cada casa enlaza a su ficha individual (`/dev/slug-modelo`): el nombre, la foto, el CTA y el pie.
> - **Peso**: las fotos del brochure NO usan las del sitio directo (daban PDF de 11 MB) → versiones JPEG ligeras (~1000-1600px, q72) en **`public/brochure-img/`**, generadas por **`scripts/gen-brochure-img.mjs`**. El componente las mapea con la función **`brImg(src)`** (misma regla de slug que el script — si se tocan, mantener sincronizadas). El QR está en `public/jds6/casas-qr.png`.
> - **⚠️ FLUJO DE REGENERACIÓN del PDF** (cuando cambien precios/fotos): (1) `node scripts/gen-brochure-img.mjs` si cambiaron imágenes; (2) `npm run build`; (3) servir `out/` local (`python3 -m http.server`); (4) puppeteer `page.pdf()` de la URL → `public/brochure-pdf/{slug}-{kind}.pdf`; (5) `npm run build` otra vez (copia el PDF a `out/`); (6) deploy. **El PDF NO se actualiza solo** (a diferencia de la página web, que siempre está al día). Para JdS6 casas los precios son estables. Script de generación del PDF quedó en el scratchpad de la sesión; conviene volverlo un `scripts/gen-brochure-pdf.mjs` versionado la próxima vez.
>
> **Estado:** desplegado OCULTO — `robots: index:false` TEMPORAL + fuera del sitemap + sin enlaces (igual que se hizo con la calculadora). Florencio lo revisa en vivo por URL directa. **✅ La página HTML está COMPLETA y lista para indexar** (contenido real, SEO, JSON-LD OfferCatalog, amenidades, responsive verificado desktop/móvil/320px). El PDF es un asset aparte que se sigue puliendo SIN bloquear la indexación.
>
> **PENDIENTES:**
> - **(a) ✅ HECHO (25 jul, sesión 47) — PÁGINA CASAS JdS6 YA PÚBLICA.** Florencio dio OK al brochure y pidió publicar. Resultó que el `index:true` (en `buildCatalogMetadata`), el sitemap (`CATALOG_PAGES` en `sitemap.ts`) y el enlace discreto desde el silo ("Ver solo las casas de Jardines del Sur 6 →" en `SiloPage.tsx`) **YA estaban** — lo único que faltaba era mostrar el botón de descarga: se quitó el `<PreviewGate param="folleto">` de `CatalogPage.tsx` (+ su import). Verificado en vivo: botón visible sin `?folleto`, robots index/follow, en sitemap, enlazada desde el silo. **2×1 SEO completo.** ⏳ Pendiente de Florencio: solicitar indexación de `/jardines-del-sur-6/casas` en Search Console.
> - **(b) ✅ HECHO (25 jul, sesión 47) — BROCHURE REDISEÑADO A ESTILO REVISTA.** Florencio eligió **REVISTA VERTICAL** (le presenté apaisado/lookbook y tríptico como alternativas con sus contras). **Razón decisiva: el folleto se comparte por WhatsApp → se abre en el CELULAR;** vertical se lee sin girar. Apaisado obliga a rotar el cel y el tríptico rompe el orden de lectura como PDF digital → descartados (pero anotados por si algún día quiere stock IMPRESO). **El problema real NO era el tamaño carta sino el "look de hoja":** las páginas de casa tenían la mitad inferior vacía. Fix aplicado en `catalog.module.css` (@media print) + markup en `CatalogPage.tsx`: cada casa es ahora una página EDITORIAL image-forward → foto a sangre completa (162mm, sin bordes/redondeos), con la MARCA arriba y el NOMBRE+PRECIO sobre la foto (sobre degradado navy), y debajo una banda blanca que LLENA el resto (chips + distribución + botón CTA dorado real + pie con QR anclado al fondo vía `margin-top:auto`). La portada NO se tocó (ya conversaba con este lenguaje). La página de amenidades: clase propia `.brAmenPage` (recupera márgenes, que `.brPage` perdió al pasar a padding:0 para la sangre), fotos a 45mm y contacto+legal anclados al fondo (`.brAmenFoot` margin-top:auto) → robusto para cualquier nº de amenidades (importa: plantilla reutilizable). **PDF regenerado: 6 págs, 1140 KB, links clicables OK.** El botón sigue OCULTO tras `?folleto` — pendiente que Florencio lo revise en vivo y decida quitar el gate. **⚠️ Script de PDF YA VERSIONADO:** `scripts/gen-brochure-pdf.mjs` (puppeteer-core + Chrome del sistema, `page.pdf` con `preferCSSPageSize`); uso: `node scripts/gen-brochure-pdf.mjs <slug> <kind> <puerto>` sirviendo `out/` (flujo completo documentado en el header del script).
>   - **Ronda 2 de feedback de Florencio (25 jul, mismo día) — 3 ajustes aplicados y verificados:** (1) **PORTADA más resaltable:** fila de amenidades destacadas en píldoras con borde dorado sobre la foto (`.brCoverHighlights`, toma `amenitiesSection.items.slice(0,5)`). (2) **FOTOS de casa más grandes:** subí `.brPhotoLink` de 162mm → **178mm** (casi cuadrada, llena el hueco bajo el CTA que él notó); verificado que Ceiba (3 niveles, la de más texto) sigue entrando. (3) **⭐ WhatsApp abre WhatsApp, NO la web:** el número era texto/parte de un link a la web; ahora en los 3 lugares el número es un `<a href="wa.me/...">` propio — en las páginas de casa con **mensaje pre-llenado por modelo** (`waModelHref`: "me interesa la {modelo}... ({precio})"), en portada y tarjeta de contacto con el `waHref` general. Number en **verde** para que se lea como WhatsApp. Requirió deshacer el link único que envolvía todo (no se pueden anidar `<a>`): el pie de casa pasó a `.brFootText` con 2 links (web + wa), y la tarjeta de amenidades de `<a>` contenedor → `<div>` con URL y WhatsApp como links separados. PDF regenerado (1164 KB). Sigue oculto tras `?folleto`.
> - **(c) Breadcrumb del navbar** en esta ruta: hoy `SiteHeader` cae al segmento crudo ("casas") porque no reconoce la ruta. Pulir el mapeo.
> - **(d) REPLICAR a las demás** con la misma plantilla: Departamentos JdS6, y sumar amenidades+PDF a La Rioja/Lirios/JdS7.
>
> **Nota de flujo:** todo el trabajo de la sesión está SIN commit (Florencio commitea). Archivos nuevos: `CatalogPage.tsx`, `catalog.module.css`, `jardines-del-sur-6/casas/`, `public/brochure-img/`, `public/brochure-pdf/`, `public/jds6/` (QR), `scripts/gen-brochure-img.mjs`; modificado `dev-meta.ts`. Se borró `components/PrintButton.jsx` (reemplazado por la descarga del PDF).

> **✅ PUBLICADA sesión 45 (24 jul 2026) — CALCULADORA HIPOTECARIA YA ESTÁ EN VIVO Y PÚBLICA (`/calculadora-hipotecaria`). Checklist de publicación EJECUTADO:** (1) quitado `robots: index:false` → ahora `index:true` + googleBot con max-image-preview/max-snippet (como las demás páginas); (2) agregada a `app/sitemap.ts` (priority 0.7, tras el home/devs, antes de FAQ); (3) enlace **"Calculadora"** agregado a los **6 footers** (home, blog índice, blog artículo, FAQ, JdS7 y la propia calculadora) — insertado entre "Preguntas Frecuentes" y "Blog"; (4) build + verificado en el HTML compilado (sin noindex, en sitemap.xml, enlace presente) + deploy. **⚠️ PENDIENTE que hace Florencio: solicitar indexación de `/calculadora-hipotecaria` en Search Console.** Decisión de Florencio: publicar YA aunque no terminó de revisar en celular ("después le corrijo si veo algún detalle"). **NO se hizo:** ampliar el SEO del title para búsquedas genéricas (quedó pendiente de decisión, ver abajo) ni tocar el header transparente. El mini-formulario de leads sigue sin estrenarse.
>
> **✅ (sesión 45, 24 jul) — "Calculadora" AGREGADA AL NAVBAR** (pedido de Florencio). En `SiteHeader.jsx`, 3 lugares: (1) menú de escritorio de páginas NO-silo (home/blog/FAQ/calculadora) entre "Amenidades" y "Contacto"; (2) drawer móvil del home (04 Calculadora, Contacto→05); (3) drawer móvil de silos/modelos (tras Amenidades, Contacto recorrido a +3). **NO se tocó el breadcrumb de escritorio de los silos** (diseño "centro vacío premium" de la sesión 7 sigue intacto) → en escritorio, dentro de un silo/modelo, la Calculadora NO aparece; en móvil sí (vía drawer). Si Florencio quiere Calculadora en escritorio DENTRO de silos, habría que romper ese diseño premium — pendiente de su confirmación. ⚠️ **OJO con el veto viejo (sesión 36):** ese veto era SOLO del botón "Promociones" en el navbar (y fue un malentendido), NO un veto general al navbar — agregar la Calculadora fue una petición explícita y legítima.

> **✅ (sesión 45, 24 jul) — SEO / "Site Name" y Sitelinks (duda de Florencio al ver que la competencia clon sale con nombre de marca + sitelinks y él no):** Se AUDITÓ el código y **TODO el SEO técnico ya estaba bien hecho** — `WebSite` JSON-LD con `name` (page.tsx), `og:site_name` (layout.tsx), `<title>` con la marca al frente, sitemap.xml limpio, `BreadcrumbList` en las 8 plantillas internas. Lo ÚNICO que se agregó: `alternateName: ['Altta Homes','Altta Homes Cancun','AlttaHomes']` al nodo `WebSite` (Google lee name+alternateName de ahí para el Site Name). **Diagnóstico real (importante, NO re-litigar como si faltara código): el sitio NO sale con Site Name ni sitelinks porque el dominio `.mx` es NUEVO (~5 semanas, migró el 20 jun) — es cuestión de TIEMPO y AUTORIDAD, no de código.** Los sitelinks son 100% automáticos, no se activan. Lo que acelera (tarea de Florencio, no de código): búsquedas de marca "Altta Homes Cancún" → clic al sitio, reseñas en Google Business, y el tráfico que ya generan las campañas. La respuesta que le dio Gemini era correcta en general pero le pedía agregar cosas que YA existían.

> **🚧 (histórico del 23 jul, sesión 44) — CALCULADORA HIPOTECARIA: sesión larga de pulido con Florencio revisando en vivo.**
>
> Contexto: la página `/calculadora-hipotecaria` ya existía (construida sesiones 39-42, calibrada contra BBVA). Esta sesión fue de pulido guiado por Florencio, que iba probando en su pantalla y cazando errores. **Modo de trabajo que funcionó: Florencio manda captura → se corrige un detalle concreto → build + deploy → él revisa el siguiente.** Cambios hechos (todos en `components/MortgageCalculator.jsx` + `.module.css` y `app/calculadora-hipotecaria/page.tsx` + `calculadora.module.css`):
>
> **A) SELECTOR DE MODELO:** se cortaba en celular ("Casa Ceiba · Jardines del Sur 6 — $2,150,000" no cabía). Solución: agrupado por desarrollo con `<optgroup>` (el dev va de encabezado, cada opción solo el nombre del modelo; el precio ya se ve en el desglose). + font-size 16px en móvil (bajo 16px iOS hace zoom automático al enfocar y descuadra; en desktop baja a 0.85rem por media query — mismo truco aplicado a los inputs manuales).
>
> **B) TÉRMINOS ALINEADOS CON EL RESTO DEL SITIO:** el desglose ahora dice **"Valor avalúo"** y **"Precio con descuento"** (antes "Avalúo bancario" / "Precio del modelo") — es EXACTAMENTE lo que muestran las tarjetas `PropertyCard` de todo el sitio; el cliente ve el mismo lenguaje en la ficha y en la calculadora. Orden: avalúo arriba, precio abajo (cuánto VALE antes de cuánto PAGA → el ahorro salta). Cada renglón lleva un subtítulo chico explicativo ("Su valor comercial: lo que realmente vale" / "Lo que pagas por ella").
>
> **C) ⚠️ PATRÓN DE BUG REPETIDO 3 VECES (importante para el futuro): rótulos de texto FIJO describiendo números que VARÍAN.** Florencio cazó los tres:
>   1. "90% del valor avalúo" cuando el crédito real era 89.94% → al multiplicar 0.90×avalúo salían $1,700 de diferencia. Fix: el % se calcula (crédito÷avalúo) con `fmtPct` a **2 decimales** (con 1 decimal aún se iba $1,060; con 2, ~$44 = redondeo puro).
>   2. "Cubre precio y gastos" en el subtítulo del crédito — MENTÍA en los 4 modelos que se pasan del 90% (Flamboyán, Casa Noni JdS6, Casa Noni La Rioja, Cedro Plus Lirios): ahí el crédito NO cubre todo, el faltante lo pone el cliente en efectivo. Fix: `creditSub` con 3 estados (sin aportación / topado en 90% / aportación voluntaria).
>   3. En modo manual, con el avalúo vacío se usaba el precio de venta PERO se seguía rotulando "% del avalúo" → calculaba sobre el precio y lo llamaba avalúo. Fix: sin avalúo NO se calcula (se pide el dato); se quitó el respaldo silencioso.
>   **Regla aprendida: un cálculo mal etiquetado es PEOR que ningún cálculo** (el cliente se lo lleva a WhatsApp como verdad). Cuando un caso sea "raro" (falta un dato, número no redondo), DECIRLO, no taparlo con un default.
>
> **D) NUEVO — MODO MANUAL "otra propiedad"** (idea de Florencio, doble beneficio: sirve para las variantes de depto que no están en `inventory.json` y para CUALQUIER propiedad → además abre la página a búsquedas genéricas de "calculadora hipotecaria", mucho más volumen). En el mismo `<select>`, grupo "Otra propiedad" → "Ingresar mis propios valores" → despliega 2 campos (Precio de venta + Valor de avalúo) SIN cambiar de página. Detalles: campos con formato de dinero en vivo ("2,500,000" con comas, vía `digitsOnly`/`fmtGrouped`, type="text" inputMode="numeric" — un type="number" no pinta separadores); placeholder "Ej. 2,500,000" en gris/cursiva (antes "2,800,000" a secas se confundía con valor capturado); **NO se calcula hasta tener AMBOS datos** (avalúo obligatorio, sin respaldo silencioso); si no lo conoce, se le pide escribir el mismo precio de venta ("cálculo conservador" — subestima crédito, sobreestima efectivo, nunca sorprende de más); el avalúo SÍ se pinta en el desglose aunque sea igual al precio (ocultarlo hacía "desaparecer" el dato que acababa de escribir); mensaje de WhatsApp adaptado ("hice el ejercicio con mis propios valores").
>
> **E) COPY DE LA PÁGINA (`page.tsx`):** (1) sección nueva "¿Cómo es posible estrenar sin enganche?" reescrita: el banco presta sobre el **valor avalúo = valor comercial** (lo que la vivienda realmente vale, por perito, **"no por nosotros"** — quita la sospecha de precio inflado); (2) sección nueva **"¿Qué tan cerca está este número del real?"** — el orden IMPORTA: primero admite el límite (el monto exacto lo define el banco con tu tabla de amortización real), DESPUÉS ofrece la solución (podemos compartir tablas de amortización con mensualidades reales de los bancos); admitir el límite es lo que da credibilidad; (3) glosario en tarjetas (`.glossary`) — quedó en 2 términos: **"Valor de avalúo"** (quitado "no según el vendedor" → Florencio ES el vendedor; ahora "No es lo que tú pagas") y **"Precio neto"** (el precio que da el desarrollador ya con descuentos, o el acordado en compra directa). Se ELIMINÓ "Precio de lista" (sobra: el cliente nunca lo paga). ⚠️ **Se corrigió una PROMESA FALSA en 2 lugares:** "si preguntas por otra propiedad te ayudamos a estimarlo sin costo" / "te lo averiguamos sin costo" → NO es cierto (un avalúo lo hace un perito en el trámite, no Florencio). Reemplazado por "esta calculadora usa valores de avalúo recientes de cada vivienda, para coherencia y formalidad" y en el modo manual "escríbenos y lo revisamos juntos".
>
> **PENDIENTES POST-PUBLICACIÓN (la calculadora YA está en vivo, esto es afinación):**
> - ⏳ **Florencio solicita indexación de `/calculadora-hipotecaria` en Search Console** (lo hace él).
> - ⏳ **Florencio revisa la calculadora en CELULAR** — no alcanzó a terminar antes de publicar. Aún sin validar en móvil: sliders con el pulgar, botones de plazo, panel oscuro apilado, los 2 campos manuales lado a lado que a <520px apilan a 1 columna. Si ve algún detalle, se corrige sobre lo ya publicado.
> - ⏳ **DECISIÓN header transparente:** en `/calculadora-hipotecaria` el título se transparenta a través del navbar de vidrio (blanco 82% + blur) al hacer scroll — se nota más aquí por el bloque azul sólido detrás. Opciones dadas: (1) header sólido al scroll, (2) subir opacidad a 92-95%, (3) dejarlo. **Afecta las ~15 páginas del sitio (`SiteHeader.module.css` `.scrolled`), NO tocar sin decisión de Florencio.** Recomendación: opción 2 si le molesta.
> - ⏳ **DECISIÓN SEO:** ¿ampliar `metaTitle`/`metaDescription` (hoy solo Cancún + modelos) para pescar búsquedas genéricas "calculadora hipotecaria México"? Ahora que el modo manual sirve para cualquier propiedad tendría sentido, pero trae tráfico menos calificado. Pendiente que Florencio decida.
> - ⏳ ¿Bloque compacto de la calculadora en las fichas de MODELO? La prop `initialModelId` ya está soportada. No se ha hecho.
> - Recordatorio: el **mini-formulario de leads** (sesión 42) iba a estrenarse JUNTO con la calculadora — NO se hizo. Decidir si entra después (falta correo destino + servicio de envío de email).

> **🔧 CORRECCIÓN sesión 44 (23 jul 2026) — Copy de la galería de obra de JdS7 (2 cambios pedidos por Florencio), desplegado y verificado en vivo:**
>
> 1. **⚠️ REGLA DE NEGOCIO NUEVA: los clientes NO pueden visitar la obra de JdS7.** El recuadro dorado junto a la galería decía "¿Quieres ver la obra en persona? / Agenda tu visita por WhatsApp" — prometía algo imposible. Reemplazado por **"¿Quieres ser de los primeros? / Pregunta por WhatsApp"** (se conservó el CTA en vez de borrarlo: la galería es el punto de máximo interés del visitante; el gancho ahora es la preventa, no la visita). **NO volver a ofrecer visitas/recorridos a la obra de JdS7 en ningún copy, anuncio ni mensaje prellenado.**
> 2. **Crédito de las fotos:** "Fotos tomadas en sitio por tu asesor el 23 de julio de 2026" → **"Fotos tomadas el 23 de julio de 2026"** (Florencio no quiere la atribución al asesor en el texto visible). El JSON-LD de `ImageGallery` conserva "tomadas en sitio el {fecha}" — es metadato para Google, no texto visible.
>
> Ambos textos viven en `app/(desarrollos)/jardines-del-sur-7/page.tsx` (sección `#avance-de-obra`, ~líneas 337-352). Build + deploy + verificado en producción.
>
> **2-bis. ✅ Indexación de `/jardines-del-sur-7` YA SOLICITADA en Search Console** (Florencio, 23 jul 2026). Ese pendiente queda cerrado; ahora solo toca esperar a que Google la recoja.
>
> **2-ter. 📁 Carpeta de fotos para el perfil de Google Business — ENTREGADA:** `D:\Fotos JdS7 - Subir a Google Business\` (FUERA del repo, no se commitea ni despliega). Contiene las MISMAS 9 fotos de la galería web convertidas de **WebP a JPG** — ⚠️ **Google Business NO acepta WebP**, pide JPG/PNG, mínimo 720×720 y entre 10 KB y 5 MB; las exportadas son 1600px y 284-399 KB, todas cumplen. Van numeradas 01-09 en el mismo orden de la galería (la 01 primero: en GBP la primera suele quedar de portada) e incluyen un `LEEME.txt` con el orden y las reglas. Script reutilizable en el scratchpad (`export-gbp.js`, usa sharp con `NODE_PATH` al node_modules del proyecto). **Si algún día se cambian las fotos de la galería, regenerar esta carpeta con el mismo script.**
>
> **3. ✅ RESUELTO el pendiente del CP de JdS7 (77500 vs 77536) — SE QUEDA EN 77500, NO cambiarlo:** el CP oficial del fraccionamiento Jardines del Sur SÍ es 77536 (verificado con Correos/micodigopostal), **pero Google Business RECHAZA esa dirección** — Florencio lo intentó y responde *"dirección no encontrada"*: la calle Josefa Ortiz de Domínguez en esa manzana es nueva y aún no está en la base de direcciones de Google con ese CP (normal en desarrollos en obra). Como el perfil quedó forzosamente en 77500, **la web debe COINCIDIR con el perfil**: la consistencia de NAP (nombre-dirección-teléfono) pesa más en SEO local que el CP exacto, y quien realmente posiciona el pin en Maps son las COORDENADAS (21.0835955, −86.8914348), que sí están correctas. ⚠️ **NO "corregir" a 77536 en sesiones futuras** — hay un comentario de advertencia en el código junto a `JDS7_POSTAL`. Reintentar en AMBOS lados (perfil + web) cuando Google reconozca la calle. Los otros 3 desarrollos siguen en 77536, y está bien: son direcciones distintas.
> - **🔔 ACUERDO CON FLORENCIO (23 jul 2026):** él reintentará el 77536 en Google Business cada cierto tiempo; **cuando Google se lo acepte, él avisa y entonces se cambia `JDS7_POSTAL` a "77536" en la web** (un solo lugar, `page.tsx`) + se quita el comentario de advertencia. Hasta que él lo diga, NO tocarlo.
>
> **4. Limpieza:** se eliminó la constante `JDS7_ADDRESS` (declarada en la sesión 43 y nunca usada — el texto visible y el JSON-LD arman la dirección con `JDS7_STREET` + `JDS7_POSTAL` por separado). Cero cambio de salida.
>
> **Nota sobre las fotos de obra (pregunta de Florencio):** las 18 imágenes YA están en WebP y en 2 niveles — **9 miniaturas 640px = 244 KB en total** (lazy, es lo único que baja quien solo hace scroll) y 9 grandes 1600px = 1.9 MB que **solo se descargan al abrir el lightbox, y solo la que se abre** (~200 KB). No hay nada que optimizar ahí.
>
> **⚠️ LECCIÓN DE FLUJO (importante):** esta sesión empezó con el sitio EN VIVO por delante del repo local — la sesión 43 se trabajó en OTRA computadora y el commit se hizo pero **nunca se pusheó** (`git ls-remote origin` lo confirmó: GitHub seguía en `6283c0f`). Si se hubiera editado y desplegado desde el repo local, se habrían BORRADO la galería de obra y la sección de ubicación del sitio en vivo. **Regla: al trabajar en más de una máquina, hacer `git push` (no solo `commit`) al cerrar cada sesión; y al abrir sesión, si algo que se ve en producción no está en el código local, PARAR y sincronizar antes de tocar nada.**

> **✅ COMPLETADO sesión 43 (23 jul 2026) — JARDINES DEL SUR 7: galería de AVANCE DE OBRA (fotos reales) + sección de UBICACIÓN vinculada a su perfil de Google Business. Desplegado, commiteado y pusheado (`ebdc57a` + `51de8e3`):**
>
> Florencio fue a la obra, tomó **38 fotos** (en `../Fotos Jardines del Sur 7/`, fuera del repo) y creó el **perfil de Google Business propio de JdS7**. Se montaron dos secciones nuevas en `/jardines-del-sur-7`.
>
> **A) AVANCE DE OBRA (`#avance-de-obra`):** 9 fotos curadas de las 38, en `public/jds7/obra/` en 2 tamaños — miniatura 640px (**236 KB las 9 juntas**, lazy) y 1600px que SOLO se descarga al abrir el lightbox. Componente nuevo **`components/ObraGallery.jsx`** (+`.module.css`): rejilla con la 1ª foto destacada 2×2, lightbox por `createPortal`, flechas, teclado ←/→/Esc y clic fuera. ⚠️ **La rejilla cierra sin huecos SOLO con 9 fotos** (4 celdas de la destacada + 8 = 12 = 3 filas × 4 col en desktop, y 4 filas × 2 col en móvil) — si algún día se cambia el número, recalcular o quedan huecos.
> - **⚠️ REGLA NUEVA DE FLORENCIO: NINGUNA foto con personas.** Pidió quitar una con cuadrillas caminando; al revisar TODAS a tamaño completo aparecieron trabajadores en azoteas y dentro de cabinas de maquinaria **que no se veían en miniatura** → se reemplazaron 3 fotos y se recortó el 12% izquierdo de la fila de townhouses (tenía una persona agachada al borde). **Verificar SIEMPRE a tamaño completo (≥1100px) antes de publicar fotos de obra nuevas.** Las 4 retiradas se borraron y los reemplazos llevan nombres NUEVOS (obra-10…13) para que nadie con caché vea las viejas.
> - **Decisión de diseño (regla reutilizable):** el **hero se queda con la foto aspiracional** (alberca) y las fotos de obra van ABAJO en su sección. La obra en gris no vende como primera impresión; sirve como prueba de avance. La nota "imagen ilustrativa" del hero ahora lleva enlace de anclaje "Ver fotos reales de la obra ↓".
>
> **B) UBICACIÓN (`#ubicacion`) — datos REALES del perfil (leídos de él, NO inventados):** dirección **Josefa Ortiz de Domínguez, 77500 Cancún, Q.R.**; coordenadas **21.0835955, −86.8914348**; place id `0x8f4c2d985cecc7db:0x39666beb394ecc4c`; kgmid `/g/11ntdw094n`; link corto `https://maps.app.goo.gl/gNUPBCHn2StkA6hB9`. El perfil ya apunta a alttahomescancun.mx y al 998 205 9044. Componente nuevo **`components/MapEmbedCard.jsx`**: el mapa **no carga hasta que el visitante hace clic** (verificado: 0 peticiones a Google al cargar la página) → no toca el PageSpeed.
> - **⚠️ GOTCHA del embed:** con `?q=<lat,lng>` Google pinta un pin SIN ficha y al tocarlo responde **"Error al cargar la información del lugar"**. La forma correcta (probada contra 4 variantes en iframes reales) es **`?q=<NOMBRE del lugar>&ll=<lat,lng>&z=16&hl=es&output=embed`** → el pin sale etiquetado "Jardines del Sur 7 | Altta Homes" con su ficha real. NO revertir a coordenadas.
> - **⚠️ CP inconsistente:** el perfil de JdS7 dice **77500**; los otros 3 desarrollos usan **77536**. Se respetó el del perfil. **Pendiente confirmar con Florencio** cuál es el correcto y alinearlos.
>
> **C) JSON-LD (lo que asocia la página con el perfil):** al nodo `Residence` se le agregó `hasMap` + `sameAs` (URL larga y corta del perfil), `geo`, `address` completo, `telephone` y las 9 fotos como `image[]`; más un nodo **`ImageGallery`** con `caption`, `datePublished: 2026-07-23` y `contentLocation` por foto.
>
> **❗ ACLARACIÓN IMPORTANTE (Florencio pidió "que salgan las imágenes del perfil en la página"):** **NO se pueden jalar automáticamente las fotos de un perfil de Google Business a la web.** No hay forma sin la API de Google Business Profile (OAuth + aprobación) y ni así está pensada para eso. Lo correcto y lo que se hizo: las fotos viven en el sitio (desde la carpeta local) y él las sube TAMBIÉN al perfil a mano; la "vinculación" real es el JSON-LD + el enlace al perfil. **No re-litigar esto en sesiones futuras.**
>
> **Pendientes de Florencio:** (a) solicitar indexación de `/jardines-del-sur-7` en Search Console (sin eso Google tarda semanas en recoger las fotos); (b) subir las 9 fotos al perfil de GBP de JdS7; (c) decidir el CP (77500 vs 77536); (d) ~~crédito de $7,000~~ ✅ **CERRADO 23 jul 2026: Florencio alcanzó los $7,000 de gasto y el crédito YA SE ESTÁ CANJEANDO.** Tema terminado — no volver a levantarlo como pendiente ni como fecha límite en sesiones futuras.

> **🔧 MANTENIMIENTO sesión 42 (18 jul 2026) — Corrección de dato: Casa Noni Elite (La Rioja 2) tenía 150 m² de terreno en el sitio; el dato real es 120 m².** Corregido `metros_terreno: 150 → 120` en `data/inventory.json` (id `larioja2-noni-elite`), build + deploy, verificado en vivo ("120 m² de terreno" en la ficha y meta description). ⚠️ Pendiente confirmar con Florencio: Fresno Elite y Modelo Álamo también dicen 150 m² de terreno — ¿esos sí son correctos? **También en sesión 42: creada la skill `.claude/skills/ads-expert/SKILL.md`** (experto Google/Meta Ads con todas las reglas duras de la cuenta — invocable con /ads-expert en sesiones futuras; incluirla en el próximo commit). **Y decisión Meta:** la campaña nueva de Ventas queda con seguimiento de eventos del sitio ON, retoques visuales OFF, solo WhatsApp; el video + imagen de depto de la "segunda campaña" van como anuncios adicionales DENTRO de la misma campaña (no como campaña aparte — era Andromeda: una campaña concentrada > varias fragmentadas); investigación de cambios Meta 2026 documentada (algoritmo Andromeda mar-2026: creativo pesa ~56%, renovar creativos cada 4-6 semanas, aprendizaje 7-14 días, juzgar a 21 días).

> **✅ COMPLETADO sesión 42 (18 jul 2026) — SESIÓN DOBLE: revisión de Meta (Events Manager) + EJECUTADA la poda de "exprimido" de Google Ads:**
>
> **A) META (revisión — CERO cambios de código necesarios):** Los scores de "Calidad de coincidencias" (PageView 5.9/10, Contact 4.4/10) son NORMALES para un sitio sin formularios — Meta pide email/teléfono que el sitio no recolecta; con IP + User-Agent + fbp al 100% (+fbc cuando hay fbclid) estamos en el máximo real posible. El aviso "deduplicación 0% / clave event_id" es COSMÉTICO: PageView va por UN solo canal (server-only por diseño desde la Fase 2b) → no existen duplicados que deduplicar; nuestros eventos SÍ llevan event_id (verificado en código). La caída de la gráfica post-13 jul = esperada (al quitar fbevents.js murieron los eventos automáticos inflados ob3/SubscribedButtonClick). **Misterio de la conexión CAPI "Activo" preexistente: RESUELTO** (eran los eventos automáticos del plugin de Meta; hoy la CAPI activa es la nuestra). Trampas descartadas: tarjeta "conecta CAPI vía GTM" (pasarela de pago — ya tenemos CAPI gratis), "generador de parámetros" (redundante) y la tarea por correo de "agregar email a PageView" (no aplica: nadie está logueado en un PageView). Los otros datasets con 0 eventos (Florencio Real Estate etc.) son inofensivos, no borrar.
>
> **DECISIÓN NUEVA: MINI-FORMULARIO de leads** (nombre + teléfono, email opcional) como canal SECUNDARIO — no por el score de Meta (los Contact de WhatsApp no cambiarían), sino por capturar al lead que no quiere iniciar chat. Se estrena JUNTO con la publicación de la calculadora hipotecaria: bloque "Recibe tu cotización" → endpoint `/api/lead` (misma Function) → evento `Lead` a CAPI con email/teléfono HASHEADOS + aviso de privacidad. WhatsApp sigue siendo el CTA principal SIEMPRE. **Pendiente del form: a qué correo recibirá Florencio los leads + conectar servicio de envío de email.**
>
> **B) GOOGLE ADS — PODA EJECUTADA** (con reportes reales: keywords 18 jun–17 jul y términos de búsqueda 12 may–17 jul). Comparación vs línea base: conversiones/día ×2 (1.0 vs 0.52) pero CPA $122.31 vs $92.11 (+33% — efecto presupuesto $150 + Max. conversiones comprando margen caro). Ejecutado por Florencio guiado:
> 1. **PAUSADAS** todas las keywords sadasi (~$240 en 2 meses, 0 conv, QS 4 — quien busca sadasi busca a la desarrolladora) y las 2 de JdS3 `[jardines del sur 3 cancun]` + `"jardines del sur 3 cancun"` (QS 3 lastre, $69, 0 conv).
> 2. **⚠️ DECISIÓN CLAVE: `[altta homes cancun]` SE QUEDA ACTIVA** pese a CPA $435 ($609 gastados, 1.4 conv) — existe un **COMPETIDOR CLON con dominio `altahomescancun.com` (una sola T) que sale primero** en la búsqueda de marca → defensa de marca justificada, es un "seguro" de ~$600/mes. **NUNCA sugerir pausarla sin este contexto.** Pendiente: investigar el clon (quién es, desde cuándo; posible reclamo de marca ante Google).
> 3. **QUITADOS los duplicados de La Rioja del Grupo de anuncios 1** (`[la rioja 2 cancun]`, `"la rioja 2"`, `[la rioja 2 altta homes]` — $100, 0 conv) → **canibalización MUERTA**: toda búsqueda rioja la atiende el grupo Premium (que sí convierte, $95-146).
> 4. **COSECHADAS 3 exactas** al Grupo 1 (quedaron "En revisión"): `[oficina de ventas jardines del sur 6]` (2 conv a **$0.47** de CPA 🤯 — la mejor búsqueda del reporte), `[jardines del sur casas en venta]` (1 conv $10), `[jardines del sur 6 venta]` (CTR 25%).
> 5. **~54 NEGATIVAS nuevas a nivel CAMPAÑA** en frase: inmobiliaria/s, renta/s, traspaso/s, administracion, opiniones, terrenos, casas baratas + todos los competidores con actividad del reporte (sense, javer, ruba, vassari, ciudad natura/maderas, turquesa, balam, sacbe, punta ikal, izamal, monteverde, malibu, altai, jade, kusamil, calakmul, urbana park, lausana, santa fe, sarabi, terraquia, palmaris, villas del mar/rey, las torres, wolf towers, gran via, ciudadela, isla diamante, real oasis, vista real, arbolada, cumbres, prado norte, urbi, cadu, trovit, kalam, lakin, sayab, mayakoba, los santos, las americas, aqua). **A propósito NO se negativó:** "cerca de mi", "fotos", "infonavit", ni JdS 1/2/4/5 (las etapas viejas sí convierten a veces: JdS2 a $27, JdS3 a $66).
>
> **C) ANUNCIOS AFINADOS (ambos RSA — quedaron "Pendiente/en revisión", normal 1-3 días):**
> - **Grupo de anuncios 1:** desc 1 "Polígono Sur"→"**Zona Sur**" (por fin los anuncios cumplen la regla [[feedback_zona_sur_no_poligono]]); desc 4 → "**Crédito bancario e Infonavit**…" (orden premium [[feedback_credito_bancario_primero]]); "Informes por **WhatsApp**" (mayúsculas); fuera 3 títulos duplicados/débiles (Hablar con un Asesor, Depas en Jardines del Sur, La Rioja 2) → entraron `Precios desde $1.75M MXN` (verificado vs inventory.json, Capua $1,758,830 — ⚠️ actualizar el título si cambia ese precio) + `Recorrido Virtual 360°` + `Agenda tu Visita Hoy`. **Las menciones de La Rioja en el anuncio general SE QUEDAN** (estrategia: el anuncio general es el del portafolio de los 3 devs; lo que se separa son las KEYWORDS).
> - **La Rioja 2 - Premium:** desc 1 → "Zona Sur" también; "Residencial de Lujo" → `Casas desde $4.04M MXN` (precalifica al lead premium); "Casa Club y Amenidades" → `Casa Club, Alberca y Pádel` (amenidades reales de la ficha); título 15 (hueco) → `Hasta 3 Niveles con Terraza`. Reservas anotadas: `A 10 Min del Aeropuerto` / `Vive en La Rioja 2 Cancún` / `3 Recámaras · Hasta 3.5 Baños`.
> - **⭐ 6 SITELINKS a NIVEL DE GRUPO para La Rioja Premium** (tapan los de campaña, que mezclaban "Depa Capua $1.7M" bajo el anuncio de lujo — pendiente desde sesión 30): La Rioja 2 Cancún (`/la-rioja-2`), Casa Fresno Elite (`/la-rioja-2/casa-fresno-elite`), Casa Modelo Álamo (`/la-rioja-2/casa-modelo-alamo`), Casa Noni Elite (`/la-rioja-2/casa-noni-elite`), Casa Noni La Rioja 2 (`/la-rioja-2/casa-noni` — texto distinto porque a nivel campaña ya existe "Casa Noni" de JdS6), Informes por WhatsApp (`/whatsapp`). URLs verificadas 200 en producción; precios reales en descripciones (⚠️ actualizar si suben los precios del inventario).
>
> **Matemática esperada de la poda:** ~$310/mes liberados de keywords con 0 conv fluyen a las MVP de ~$86 → CPA esperado de vuelta a ~$95-105 en 2-3 semanas. Re-revisar entonces: CPA, aprobación de keywords cosechadas + sitelinks + anuncios, y si el Nivel de optimización (92.5%, bajó por nuestros cambios — se ignora) muestra recomendaciones-trampa nuevas.
>
> **Pendientes de la sesión:** (a) **horario** — bajar la vista Programación de anuncios → Día y hora ANTES de recortar madrugada ([[project_lead_nighttime_conversion]]: leads de 11pm-12am SÍ cierran); (b) **stats del recurso de mensaje WhatsApp**; (c) migrar las ~85 negativas viejas de nivel grupo → campaña (5-10 min); (d) investigar `altahomescancun.com`; (e) ~~recordatorio crédito $7,000~~ ✅ **CERRADO 23 jul 2026 — meta de gasto alcanzada, crédito canjeándose.**

> **✅ (EJECUTADA en sesión 42, 18 jul — ver entrada de arriba; solo quedaron pendientes horario y stats del recurso WhatsApp) — Plan original apuntado 15 jul 2026 — SESIÓN DE "EXPRIMIDO" del presupuesto (campaña "Limitada por el presupuesto"):**
>
...
>
> **Verificado en producción (puppeteer):** `connect.facebook.net/fbevents.js` **ya NO carga** (−~245 KB de JS de terceros); beacon PageView a `/api/meta-capi` dispara en cada carga; cookies `_fbp`/`_fbc` se crean correctamente (probado con `?fbclid=`). El evento **Contact ahora va solo por canal servidor** (el código aún llama a fbq si existiera — inofensivo). **✅ MEDIDO POR FLORENCIO (14 jul): PageSpeed móvil = 91** (desktop 99/100/100/100). Cronología: baseline 84-87 → 78-79 (los logos PNG del selector, ~175 KB, se comieron la ganancia del Pixel — detectado en el audit 'Mejorar la entrega de imágenes') → logos convertidos a **WebP 128px (175→52 KB)** → **91**. Restante del audit = costo normal de fuentes propias + tags de Google (no tocar: ahí vive la conversión); 100 móvil no es meta realista con medición activa. ⚠️ El score móvil FLUCTÚA mucho entre corridas (75↔91 el mismo día, misma versión) — juzgar con 3+ corridas y tendencia, no con una. **✅ INVESTIGACIÓN LCP CERRADA (14 jul, trace propio con throttling CPU 4x + 4G vía CDP):** NO hay bug — la imagen del hero pinta **45 ms** después de descargar (el "2.9s de render delay" de PSI es artefacto de su simulación lantern; por eso el score fluctúa 75↔91). LCP real medido: **2.27s (verde)**. La descarga tarda ~1.8s no por peso (la imagen móvil pesa 31 KB) sino por CONTENCIÓN: comparte los 1.6 Mbps con ~138 KB de fuentes preloaded (5 woff2: Montserrat/Lato/Playfair) + CSS + JS. Los preloads del hero YA existían y funcionan (React 19 los izó al head ✓).
>
> **Fixes aplicados (14 jul):** (1) width/height explícitos en los logos del selector; (2) **hero ESTÁTICO en móvil** — `HeroBackground.jsx` ya no rota ni descarga los slides 2-4 en pantallas <768px (**−304 KB de datos por visita móvil**; verificado en prod: móvil baja 1 imagen, desktop conserva el carrusel de 4). **Descartado a propósito:** quitar el preload de Playfair o recortar pesos de fuentes — ahorraría ~0.3s de LCP simulado pero causa flash de fuente en los títulos (marca > décimas). El sitio está sano; juzgar velocidad por la sección de usuarios reales (CrUX) cuando aparezca, no por corridas sueltas de PSI. **RESULTADO FINAL (14 jul, medido por Florencio): PageSpeed móvil = 93** (+100/100/100 en Accesibilidad/Prácticas/SEO; desktop 99/100/100/100). Cronología del día: 84-87 → 78 (logos PNG) → 91 (logos WebP + sin Pixel) → **93** (hero estático en móvil). ⚠️ El Ken Burns (zoom) SIGUE en móvil; solo se quitó la ROTACIÓN de fotos — ✅ **HECHO (14 jul): CARRUSEL MÓVIL LIGERO de vuelta** — se generaron `vista-aerea-mobile.webp` (36 KB) y `terraza-mobile.webp` (39 KB) a 600px; `HeroBackground.jsx` ahora tiene `DEFAULT_MOBILE_SLIDES` (3 fotos: alberca/vista-aérea/terraza) que rotan en móvil montándose a los 4.2s (post-LCP). Total móvil: ~106 KB vs 335 KB originales. Los SILOS siguen estáticos en móvil (no pasan `mobileSlides`; si un día se quiere, generar variantes y pasar la prop). Verificado en prod: home móvil baja solo las 3 ligeras; silo móvil 1 imagen; desktop carrusel completo intacto. Re-medir PSI en la próxima corrida para confirmar que el 93 se sostiene. ⚠️ Si algún día se REACTIVA el Pixel en GTM, recordar que PageView se duplicaría (el tag viejo no manda event_id) — en ese caso avisar a Claude para ajustar.
...
>
> **Arquitectura desplegada:** (1) **`functions/index.js`** — Cloud Function v2 `metacapi` (Node 20, us-central1, 256MiB, maxInstances 3) expuesta como **`/api/meta-capi`** vía rewrite de Hosting (mismo dominio → sin CORS); valida método POST + whitelist de eventos (`Contact`, `Lead`) + clamps; arma payload CAPI (event_id, event_source_url, IP, user-agent, fbp/fbc) y lo manda a `graph.facebook.com/v21.0/{pixel}/events` con el token del secret; acepta `test_event_code` opcional para la pestaña "Probar eventos". (2) **`components/MetaContactTracker.jsx`** (montado en `layout.tsx`) — en cada clic a `wa.me` dispara el evento **"Contact" por DOS canales con el MISMO `event_id`**: `fbq('track','Contact',{},{eventID})` (navegador, si el Pixel de GTM cargó) + `sendBeacon` al endpoint (servidor, esquiva ad-blockers/iOS) → **Meta deduplica por event_id**. (3) Política de limpieza de imágenes: 3 días (`functions:artifacts:setpolicy`).
>
> **⚠️ DECISIÓN CLAVE — NO se manda PageView por CAPI todavía:** el Pixel de GTM manda PageView SIN event_id → mandarlo también por CAPI duplicaría conteos. El evento "Contact" es NUEVO (el Pixel solo mandaba PageView) → cero riesgo de duplicar y suma una señal de conversión que antes no existía. **Fase 2b pendiente:** migrar PageView a este esquema dual y ALIGERAR/quitar el Pixel de GTM (~245 KB) → PageSpeed móvil ~85→~95.
>
> **Verificado:** `curl POST /api/meta-capi` → `{"ok":true,"events_received":1}` (Meta lo aceptó); GET → 405; evento no permitido → 400; y con puppeteer en PRODUCCIÓN un clic real en "Hablar con un asesor" disparó el beacon. **✅ VERIFICADO POR FLORENCIO en "Probar eventos" (14 jul):** vio llegar Contact por canal Navegador (sus clics reales, IDs uuid) y por canal Servidor (evento de prueba enviado con su test_event_code al endpoint — la pestaña de pruebas SOLO muestra eventos de servidor que llevan el código; los beacons reales van a producción y se ven en Información general en ~24h). Nota: los eventos con ID `ob3_plugi…` (SubscribedButtonClick/PageView duplicado) los genera Meta solo, por la opción 'Incluye automáticamente información más detallada' del dataset — probable explicación de la conexión CAPI 'Activo' preexistente. También activó el 'Seguimiento automático' del dataset hacia sus cuentas publicitarias (atribución; no cambia optimización). **Pendiente investigar:** la conexión CAPI "Activo · Empresa conectada" preexistente del dataset (algo ya mandaba eventos de servidor — identificar qué es). **A futuro (cuando haya ~50+ Contacts):** considerar optimizar la campaña de Meta al evento Contact en vez de clics.
...
>
> **B) CAPI DE META — Fase 2 (sesión grande #2): ✅ PLAN BLAZE ACTIVADO (14 jul 2026, presupuesto de alerta $50 MXN — es alarma, no tope; gasto real esperado $0-5/mes). ✅ TOKEN DE CAPI GENERADO (14 jul 2026) — guardado en `../meta-capi-token.txt` (FUERA del repo, chmod 600; NUNCA copiarlo a archivos del repo/context). Se generó vía Dataset Quality API, SOLO para el dataset Web 2016457592282966 (se desmarcó el dataset 'Florencio Real Estate' 1532725434660745). Gotcha resuelto: Meta exigía correo de empresa registrado en Config. del negocio antes de permitir generar el token. DATO: el dataset ya muestra una conexión CAPI 'Activo' previa (Empresa conectada, recibe eventos) — investigar cuál es en la sesión del backend y deduplicar bien. AMBOS PRERREQUISITOS LISTOS → armar la función (Firebase Functions + secret + event_id dedup + Test Events).** endpoint en Firebase Functions que reenvía eventos a Meta CAPI con `event_id` para deduplicar; después aligerar el Pixel del navegador (~245 KB) → móvil ~85→~95. Prerrequisitos que hace Florencio guiado (20 min): activar plan **Blaze** + generar **token CAPI** en Events Manager. Beneficio: mejor optimización de campañas Meta (el aviso amarillo de Ads Manager "ausencia de Conversions API") + velocidad. NO trae más volumen por sí sola. ~1-2 sesiones + 1 de aligerar Pixel.
>
...
>
> **⏳ Pendientes opcionales GA4/GTM:** (a) **Vincular GA4 ↔ Google Ads** (Analytics → Administrar → Vinculaciones con Google Ads) para ver qué hace la gente tras el clic del anuncio — el que más provecho saca; (a2) **✅ HECHO (14 jul 2026) — GA4 ↔ Search Console VINCULADOS** (desde el lado de Search Console: Ajustes → Asociaciones → Asociar → propiedad GA4 'Altta Homes Cancún - Sitio Web' 543974694; verificado en 'Servicios asociados'). Falta solo que Florencio publique la colección en GA4 → Informes → Biblioteca → 'Search Console' → Publicar (datos en ~24-48h). (Instrucciones originales:) misma pantalla de Vinculaciones → elegir la propiedad de dominio `alttahomescancun.mx` + flujo web; ⚠️ después PUBLICAR los reportes en GA4 → Informes → **Biblioteca** → colección "Search Console" → Publicar (paso que casi todos olvidan; sin él los reportes quedan ocultos). Aporta: consultas orgánicas + páginas de destino cruzadas con comportamiento/conversión = termómetro de la recuperación SEO post-migración. Solo lectura, no afecta nada, datos aparecen en ~24-48 h; (b) mover la propiedad GA4 a una cuenta propia (**MOVER, no borrar**); (c) **añadir un 2º administrador** a la cuenta GTM (único aviso de diagnóstico que quedó — es solo "info", buena práctica de respaldo por si se pierde acceso, ej. agregar `fyhwin2022@gmail.com`); (d) **NO** mover la optimización de Ads a GA4 — la campaña sigue optimizando con la conversión de WhatsApp Click. GA4 acumula histórico **desde hoy (1 jul 2026)**, no antes → no revisar reportes hasta ~1 semana.
...
> - **Promo $7,000** (código `7QTVD-WKHV6-MLFY`, canjeada 31 may 2026): "gasta $7,000 → crédito $7,000". ✅ **COMPLETADA 23 jul 2026 — se alcanzó la meta de gasto y el crédito ya se está canjeando** (histórico: iba en MXN 3,052.31/7,000 al 18 jul; límite era el 30 jul). Era por **CUENTA/facturación, NO por dominio**.
...
> - **Presupuesto:** ~$40/día (confirmar). Bonus: el gasto **contaba para desbloquear el crédito de $7,000** (✅ ya completado el 23 jul 2026).
