...
>
> **B) CAPI DE META — Fase 2 (sesión grande #2): ✅ PLAN BLAZE ACTIVADO (14 jul 2026, presupuesto de alerta $50 MXN — es alarma, no tope; gasto real esperado $0-5/mes). ⏳ Falta: token de CAPI en Meta (Events Manager → dataset 2016457592282966 → Configuración → Conversions API → Generar token; guardarlo FUERA del repo, se instalará como secret de Functions) → luego armar la función.** endpoint en Firebase Functions que reenvía eventos a Meta CAPI con `event_id` para deduplicar; después aligerar el Pixel del navegador (~245 KB) → móvil ~85→~95. Prerrequisitos que hace Florencio guiado (20 min): activar plan **Blaze** + generar **token CAPI** en Events Manager. Beneficio: mejor optimización de campañas Meta (el aviso amarillo de Ads Manager "ausencia de Conversions API") + velocidad. NO trae más volumen por sí sola. ~1-2 sesiones + 1 de aligerar Pixel.
>
