...
>
> **Verificado:** `curl POST /api/meta-capi` → `{"ok":true,"events_received":1}` (Meta lo aceptó); GET → 405; evento no permitido → 400; y con puppeteer en PRODUCCIÓN un clic real en "Hablar con un asesor" disparó el beacon. **Pendiente Florencio (5 min): en Events Manager → "Probar eventos"** puede ver los Contact llegando en vivo (canal "Servidor" + "Navegador" deduplicados); en ~24h el evento Contact aparecerá en Información general del dataset. **Pendiente investigar:** la conexión CAPI "Activo · Empresa conectada" preexistente del dataset (algo ya mandaba eventos de servidor — identificar qué es). **A futuro (cuando haya ~50+ Contacts):** considerar optimizar la campaña de Meta al evento Contact en vez de clics.
