...
>
> **Verificado en producción (puppeteer):** `connect.facebook.net/fbevents.js` **ya NO carga** (−~245 KB de JS de terceros); beacon PageView a `/api/meta-capi` dispara en cada carga; cookies `_fbp`/`_fbc` se crean correctamente (probado con `?fbclid=`). El evento **Contact ahora va solo por canal servidor** (el código aún llama a fbq si existiera — inofensivo). **✅ MEDIDO POR FLORENCIO (14 jul): PageSpeed móvil = 91** (desktop 99/100/100/100). Cronología: baseline 84-87 → 78-79 (los logos PNG del selector, ~175 KB, se comieron la ganancia del Pixel — detectado en el audit 'Mejorar la entrega de imágenes') → logos convertidos a **WebP 128px (175→52 KB)** → **91**. Restante del audit = costo normal de fuentes propias + tags de Google (no tocar: ahí vive la conversión); 100 móvil no es meta realista con medición activa. ⚠️ El score móvil FLUCTÚA mucho entre corridas (75↔91 el mismo día, misma versión) — juzgar con 3+ corridas y tendencia, no con una. **⏳ PENDIENTE (investigación fina): el desglose del LCP muestra ~2.9s de 'demora de renderizado'** del hero móvil (la imagen descarga rápido pero pinta tarde) — el 1er slide ya pinta sin fade (slideActive viene del SSR), sospechosos restantes: el timeout de 4.2s que monta los slides 2-4 (loadedCount en HeroBackground.jsx), fuentes, o decode; requiere trace de Lighthouse con calma. Quick-win hecho (14 jul): width/height explícitos en los <img> de los logos (diagnóstico del reporte). ⚠️ Si algún día se REACTIVA el Pixel en GTM, recordar que PageView se duplicaría (el tag viejo no manda event_id) — en ese caso avisar a Claude para ajustar.
