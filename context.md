...
>
> **⏳ PENDIENTE post-migración. ORDEN DE PRIORIDAD (20 jun): ✅ Google Ads HECHO → ✅ Títulos SEO Fase 1 HECHO (ver abajo + `plan-seo-altta-homes.md`) → (PRÓXIMO) GBP ×3 + Fase 2 (JSON-LD: BreadcrumbList, Product/Offer) → (#2, más adelante) acortar rutas de silos. Fase 3 (autoridad/contenido: blog, reseñas, backlinks) en continuo.**
>
...
> **✅ Disclaimer legal en tarjetas (sesión 29):** agregada línea al `legalLegend` de `PropertyCard.jsx` (aplica a TODAS las tarjetas de todos los desarrollos): *"\* Aplican restricciones. Precios, promociones y disponibilidad sujetos a cambios sin previo aviso."* — protección legal estándar. Desplegado.
...
>
> **🔁 DECISIÓN (20 jun 2026) — acortar rutas de silos a `/jardines-del-sur-6` (quitar `/desarrollos-cancun/`):** Claude recomendó NO hacerlo ahora (3 veces): el SEO entre ruta larga y corta es **equivalente** (la URL casi no rankea), así que el cambio NO mejora posicionamiento; y hacerlo justo tras la migración de dominio compacta el bache de QS + re-indexación + exige 301 por ruta + re-actualizar firebase/sitemap/links. **Florencio lo entendió y aun así eligió hacerlo YA** por preferencia (quiere todo limpio), aceptando el costo/tiempo. Scope real: mover 4 carpetas de silo (+ sus `[modelo]`) de `app/desarrollos-cancun/` a la raíz, arreglar imports relativos del `_lib`, actualizar ~12 archivos con `/desarrollos-cancun/` hardcodeado (sitemap.ts, page.tsx, SiteHeader, DevelopmentTabs, SiloHero, SiloPage, ModelPage, dev-meta, blog, faq, jds7), reescribir los 26 redirects de `firebase.json` (atajos cortos → nuevas rutas) + agregar 301 de cada ruta larga vieja → corta nueva, build + deploy, re-enviar sitemap + re-solicitar indexación. Reglas al hacerlo: con guiones, sin abreviar (NADA de "jardinesdelsur6/depacapua").
